...
public class RDSJanitorResourceTracker implements JanitorResourceTracker {
    ...
    public void addOrUpdate(Resource resource) {
    	...
    	if (orig == null) {
			doInsertQuery(resource, json);
    	} else {
			doUpdateQuery(resource, json);
    	}
    	LOGGER.debug("Successfully saved.");
    }

	private void doUpdateQuery(Resource resource, String json) {
	...
	private void doInsertQuery(Resource resource, String json) {
		StringBuilder sb = new StringBuilder();
